<template>
  ...
  <Form @submit.prevent="submit">
    ...
    <keep-alive>
      <Input v-if="selectViaCep" label="CEP" v-model="location.cep_or_street" name="location.cep" type="text"
        placeholder="Digite o cep..." class="tw-mx-6 tw-mb-4" v-mask="'#####-###'" />
      <Input v-else label="Rua" v-model="location.cep_or_street" name="location.street" type="text"
        placeholder="Digite a rua..." class="tw-mx-6 tw-mb-4" />
    </keep-alive>

    <div class="tw-h-80 tw-mb-14 tw-mt-4 tw-mx-0">
      <p>Selecione o local no mapa:</p>
      <client-only>
        <LazyMap :fullscreen="false" isRegisterPoint @pointSelected="(data: LocationType) => {
    ...
    <div>
      <Input label="Adicione uma foto do local:" name="location.point.photo" type="file" accept="image/*"
        class="tw-mx-6 tw-mb-4 tw-text-wrap tw-w-[90%]" @change="handlePhotoChange" />
    </div>

    <div v-if="photoUrl">
      <h3 class="tw-text-center">Imagem selecionada:</h3>
      <img :src="photoUrl" alt="Imagem selecionada" class="tw-max-w-[15rem] tw-block tw-mx-auto" />
    </div>
