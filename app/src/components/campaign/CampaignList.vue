<template>
  <div class="campaign-list">
    ...
    <div v-else class="tw-grid tw-gap-4">
      ...
      >
        ...
        <p class="tw-text-sm tw-text-gray-600">
          {{ formatDate(campaign.startTime) }} até {{ formatDate(campaign.endTime) }}
        </p>
        <div class="tw-mt-2">
          <p class="tw-text-sm tw-font-medium">Locais:</p>
          <ul class="tw-pl-4">
            <li v-for="location in campaign.locations" :key="location.id" class="tw-text-sm">
              {{ 'Local #' + location.id }}
            </li>
          </ul>
        </div>
    ...
    <div v-if="isAdmin" class="tw-mt-6 tw-text-center">
      <Button @click="navigateTo('/campaigns/new')" class="tw-bg-blue-500 tw-text-white">
        Nova Campanha
      </Button>
    </div>
  </div>
...
onMounted(async () => {
  ...
  try {
    ...
    if (campaigns.value.length === 0) {
      ...
      console.log("Propriedades da primeira campanha:", Object.keys(firstCampaign));
    ...
    loading.value = false;
    console.log("Estado final - loading:", loading.value, "campanhas:", campaigns.value.length);
  }
});
