...
export default class CampaignService {
  ...
  try {
    ...
    if (error instanceof AxiosError) {
      ...
      if (error.response) {
        console.error('CampaignService: Resposta de erro:', error.response.data);
        ...
        console.error('CampaignService: Erro de configuração:', error.message);
      }
    } else {
  ...
  try {
    ...
    if (error instanceof AxiosError) {
      console.error(`Erro ao buscar campanha ${id}:`, error.response?.data || error.message);
    } else {
